...
from .fields import PonyConnectionField
from .converter import convert_pony_field
from .registry import Registry, get_global_registry
from .utils import get_model_fields, is_valid_pony_orm_model
...
def construct_fields(model, registry, only_fields, exclude_fields):
    ...
    for name, field in _model_fields.items():
        ...
        if field.is_collection:
            ...
            if isinstance(field.py_type, model):
                self_referenced[name] = field
                continue
        converted = convert_pony_field(field, registry)
        if not converted:
            continue
...
class PonyObjectTypeOptions(ObjectTypeOptions):
...
class PonyObjectType(ObjectType):
    @classmethod
    def __init_subclass_with_meta__(
        ...
        **options
        ...
        assert is_valid_pony_orm_model(model), (
        ...
        pony_fields = yank_fields_from_attrs(
        ...
        if connection_field_class is not None:
            ...
            connection_field_class = PonyConnectionField

        if _meta:
            assert isinstance(_meta, PonyObjectTypeOptions), (
                "_meta must be an instance of PonyObjectTypeOptions, "
                "received {}"
            ).format(_meta.__class__)
        else:
            _meta = PonyObjectTypeOptions(cls)

        _meta.model = model
        _meta.registry = registry
        _meta.fields = pony_fields
        ...
        super(PonyObjectType, cls).__init_subclass_with_meta__(
        ...
        if not skip_registry:
            ...
            if converted_fields:
                ...
                cls._meta.fields.update(pony_fields)
                registry.register(cls)
    ...
    def rescan_fields(cls):
        ...
        for field in pony_fields:
            if field not in cls._meta.fields:
                cls._meta.fields.update({field: pony_fields[field]})
    ...
    def is_type_of(cls, root, info):
        ...
        #     return True
        if not is_valid_pony_orm_model(type(root)):
